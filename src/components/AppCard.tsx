  ...
  return (
    ...
    >
      <div className="min-h-full flex items-center justify-center p-4 py-8">
        <div
          className="glass-modal relative w-full max-w-lg p-8"
          style={{ borderColor: `${app.accentColor}25` }}
        >
          {/* Close button */}
          <button
            onClick={onClose}
            className="absolute top-4 right-4 w-8 h-8 flex items-center justify-center rounded-full text-gray-400 hover:text-white transition-all z-10"
          ...
          <div className="flex items-center gap-1.5 mb-4">
            <span className="w-2 h-2 rounded-full" style={{ background: status.dot, boxShadow: `0 0 6px ${status.dot}` }} />
            <span className="text-xs font-semibold tracking-wider" style={{ color: status.text }}>{status.label}</span>
          </div>

          {/* Icon + Name */}
          <div className="text-4xl mb-2">{app.icon}</div>
          <h2 className="text-2xl font-bold text-white mb-1">{app.name}</h2>
          <div className="text-sm font-medium mb-4" style={{ color: app.accentColor }}>{app.suiteName}</div>
          ...
          {app.url ? (
            <a
              ...
              className="mt-6 flex items-center justify-center gap-3 p-4 rounded-xl transition-all duration-300 group"
              style={{ background: `linear-gradient(135deg, ${app.accentColor}15, ${app.accentColor}08)`, border: `1px solid ${app.accentColor}30` }}
            ...
            >
              <span className="text-lg font-semibold text-white group-hover:text-cyan-300 transition-colors">Visit Platform &rarr;</span>
            </a>
          ) : app.status === 'testflight' ? (
            <div className="mt-6 flex items-center justify-center gap-3 p-4 rounded-xl" style={{ background: 'rgba(30,41,59,0.5)', border: '1px solid rgba(59,130,246,0.2)' }}>
              <span className="text-blue-400 text-sm font-medium">Available on TestFlight</span>
            </div>
          ) : (
            <div className="mt-6 flex items-center justify-center gap-3 p-4 rounded-xl" style={{ background: 'rgba(30,41,59,0.5)', border: '1px solid rgba(51,65,85,1)' }}>
  ...
  return (
    <>
      <div
        ...
        style={{ borderTop: `2px solid ${app.accentColor}40` }}
      >
        <div className="relative z-10">
          {/* Status badge */}
          <div className="absolute top-0 right-0 flex items-center gap-1.5">
            <span className="w-1.5 h-1.5 rounded-full" style={{ background: status.dot, boxShadow: `0 0 4px ${status.dot}` }} />
            <span className="text-[10px] font-semibold tracking-wider" style={{ color: status.text }}>{status.label}</span>
          </div>
